...
class PlannerToolUseTests(unittest.TestCase):
    ...
    def test_system_prompt_mentions_write_file_tool(self) -> None:
        ...
        self.assertIn("Questions that list, inspect, search, or determine files", prompt)
        self.assertIn("Questions that list, inspect, search, or determine files", LOCAL_AGENT_PROMPT)
        self.assertIn("Do not open the path, launch a browser", prompt)
        self.assertIn("Do not open the path, launch a browser", LOCAL_AGENT_PROMPT)
        self.assertIn("directories discovered by `system_info`", prompt)
        self.assertIn("directories discovered by system_info", LOCAL_AGENT_PROMPT)
    ...
    def test_tool_registry_exposes_edit_and_delete_tools(self) -> None:
        ...
        desktop_observe = next(schema for schema in registry.schemas() if schema["name"] == "desktop_observe")
        desktop_resolve = next(schema for schema in registry.schemas() if schema["name"] == "desktop_resolve")
        list_path = next(schema for schema in registry.schemas() if schema["name"] == "list_path")
        system_info = next(schema for schema in registry.schemas() if schema["name"] == "system_info")
        self.assertIn("actual filesystem", list_path["description"])
        self.assertIn("user directories", system_info["description"])
        self.assertIn("browser transfer status", desktop_observe["description"])
        self.assertIn("explicitly asked to open", desktop_action["description"])
    ...
    def test_close_window_rejects_dispatched_but_unverified_result(self) -> None:
        ...
        self.assertEqual(result["reason"], "desktop_action_verification_failed")
